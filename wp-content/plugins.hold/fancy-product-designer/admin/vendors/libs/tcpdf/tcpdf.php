...
class TCPDF {
	 ...
	 * @protected
	 */
	protected $svgviewport_x = 0; // FPD

	/**
	 * SVG Viewport Y
	 * @protected
	 */
	protected $svgviewport_y = 0; // FPD
	...
	protected function Transform($tm) {
		...
		if ($this->inxobj) {
			// we are inside an XObject template
			echo '<script>console.log("Hello1", ' . json_encode($tm) . ')</script>';
			if (end($this->xobjects[$this->xobjid]['transfmrk']) !== false) {
			...
			//echo '<script>console.log("Hello2", ' . json_encode($tm) . ')</script>';
			$key = key($this->transfmrk[$this->page]);
			//$this->transfmrk[$this->page][$key] = $this->pagelen[$this->page];
			//echo '<script>console.log("Hello2", ' . json_encode($this->transfmrk) . ')</script>';
		}
	}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	*/
	...
	public function ImageSVG($file, $x='', $y='', $w=0, $h=0, $link='', $align='', $palign='', $border=0, $fitonpage=false) {
		...
		$this->svgviewport_x = $ox; // FPD
		$this->svgviewport_y = $oy; // FPD
		if ($ow <= 0) {
			$ow = 1;
		}
		if ($oh <= 0) {
	...
	protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		switch($name) {
			...
			case 'rect': {
				...
				if ($clipping) {
					$this->SVGTransform($tm);	// FPD
					$c_tm = $this->convertSVGtMatrix($tm); // FPD
					
					$this->RoundedRectXY($x, $y, $w, $h, $rx, $ry, '1111', 'CNZ', array(), array()); // FPD
					$o_tm = $this->convertSVGtMatrix([$tm[0], -$tm[1], -$tm[2], $tm[3], -$tm[4], -$tm[5]]); // FPD
					$e = ((1 - $c_tm[0] * $c_tm[0] + $c_tm[1] * $c_tm[2]) * $this->svgviewport_x) + (($c_tm[2] * $c_tm[3] - $c_tm[0] * $c_tm[2]) * $this->svgviewport_y) + $c_tm[2] * $c_tm[5] - $c_tm[0] * $c_tm[4]; // FPD
					$f = ((1 - $c_tm[3] * $c_tm[3] + $c_tm[1] * $c_tm[2]) * $this->svgviewport_x) + (($c_tm[0] * $c_tm[1] - $c_tm[1] * $c_tm[3]) * $this->svgviewport_y) + $c_tm[1] * $c_tm[4] - $c_tm[3] * $c_tm[5]; // FPD
					$this->_out(sprintf('%F %F %F %F %F %F cm', $o_tm[0], $o_tm[1], $o_tm[2], $o_tm[3], $e, $f)); // FPD
