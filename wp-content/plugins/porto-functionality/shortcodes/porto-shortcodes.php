...
class PortoShortcodesClass {
	...
	public function save_meta_values( $post_id, $post ) {
		...
		// save dynamic styles
		if ( false !== strpos( $post->post_content, '<!-- wp:porto' ) && false !== strpos( $post->post_content, '"style_options":{' ) ) { // Gutenberg editor
		...
		foreach ( $blocks as $block ) {
			if ( ! empty( $block['blockName'] ) && 0 === strpos( $block['blockName'], 'porto' ) ) {
				...
				if ( ! empty( $atts ) && ! empty( $atts['style_options'] ) ) {
					...
					$settings['selector'] = '.porto-gb-' . self::get_global_hashcode( $settings, str_replace( 'porto/porto-', '', $block['blockName'] ) );
	...
	public function gb_elements_wrap_class_filter( $class_string, $atts, $name ) {
		if ( is_array( $atts ) ) {
			if ( ! empty( $atts['style_options'] ) ) {
				$class_string .= ' porto-gb-' . self::get_global_hashcode( $atts['style_options'], $name );
...
require_once dirname( PORTO_SHORTCODES_PATH ) . '/lib/blocks/porto-blocks.php';
