...
if ( ! class_exists( 'PortoBuildersType' ) ) :
	class PortoBuildersType {
		...
		public function elements_wrap_class_filter( $class_string, $atts, $name ) {
			...
			if ( is_array( $atts ) ) {
				if ( ! empty( $atts['font_settings'] ) ) {
					if ( ! empty( $atts['alignment'] ) ) {
						$atts['font_settings']['textAlign'] = $atts['alignment'];
					}
					$class_string .= ' porto-gb-' . PortoShortcodesClass::get_global_hashcode( $atts['font_settings'], $name );
				}
				if ( 'porto-tb/porto-featured-image' == $name ) {
				...
				} elseif ( ( 'porto-tb/porto-meta' == $name || 'porto-tb/porto-woo-buttons' == $name ) && isset( $atts['spacing'] ) && ( $atts['spacing'] || '0' == $atts['spacing'] ) ) {
					$extra_cls = 'porto-gb-' . PortoShortcodesClass::get_global_hashcode( $atts, $name );
					if ( false === strpos( $class_string, $extra_cls ) ) {
						$class_string .= ' ' . $extra_cls;
					}
			...
			foreach ( $blocks as $block ) {
				if ( ! empty( $block['blockName'] ) && in_array( $block['blockName'], array( 'porto-tb/porto-content', 'porto/porto-heading', 'porto/porto-button', 'porto-tb/porto-woo-price', 'porto-tb/porto-woo-rating', 'porto-tb/porto-woo-stock', 'porto-tb/porto-woo-desc', 'porto-tb/porto-woo-buttons', 'porto-tb/porto-meta' ) ) ) {
					...
					if ( ! empty( $atts ) && ( ! empty( $atts['font_settings'] ) || ! empty( $atts['alignment'] ) ) ) {
						...
						$settings['selector'] = '.porto-gb-' . PortoShortcodesClass::get_global_hashcode( $settings, str_replace( 'porto/porto-', '', $block['blockName'] ) );
						if ( 'porto-tb/porto-woo-price' == $block['blockName'] ) {
							...
							$settings['selector'] .= ' p';
						} elseif ( 'porto/porto-heading' == $block['blockName'] ) {
					...
					if ( ! empty( $atts['show_content_hover'] ) ) {
						$atts['selector'] = '.porto-gb-' . PortoShortcodesClass::get_global_hashcode( $atts, str_replace( 'porto/porto-', '', $block['blockName'] ) ) . ' .tb-hover-content';
