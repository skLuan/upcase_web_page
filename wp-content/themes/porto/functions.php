...
function porto_css() {
	...
	if ( file_exists( $upload['basedir'] . '/redux/custom_fonts/fonts.scss' ) && filesize( $upload['basedir'] . '/redux/custom_fonts/fonts.scss' ) ) {
		wp_register_style( 'redux-custom-fonts-css', $upload['baseurl'] . '/redux/custom_fonts/fonts.scss', '', filemtime( $upload['basedir'] . '/redux/custom_fonts/fonts.scss' ), 'all' );
	...
	if ( defined( 'ELEMENTOR_VERSION' ) && is_singular() ) {
		if ( 'internal' !== get_option( 'elementor_css_print_method' ) ) { // external
			...
			if ( is_array( $inline_styles ) && ! empty( $inline_styles ) && is_singular() && get_post_meta( get_the_ID(), '_elementor_edit_mode', true ) ) {
