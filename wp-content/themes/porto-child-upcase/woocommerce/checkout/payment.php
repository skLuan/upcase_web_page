...
<div id="payment" class="woocommerce-checkout-payment">
	...
	<?php if ( porto_checkout_version() == 'v2' ) : ?>
		<h3><?php esc_html_e( 'Métodos de pago', 'woocommerce' ); ?></h3>
	<?php endif; ?>

	<?php if ( WC()->cart->needs_payment() ) : ?>
		<div class="porto-separator m-b-md"><hr class="separator-line align_center"></div>
		<h4 class="px-2"><?php esc_html_e('Métodos de pago', 'woocommerce' ); ?></h4>
		<ul class="px-2 wc_payment_methods payment_methods methods">
			<?php
	...
	<div class="form-row place-order">
		...
		<?php echo apply_filters( 'woocommerce_order_button_html', '<button type="submit" class="py-3 mt-3 button alt btn-v-dark w-100" name="woocommerce_checkout_place_order" id="place_order" value="' . esc_attr( $order_button_text ) . '" data-value="' . esc_attr( $order_button_text ) . '">'. esc_html( $order_button_text ) .'</button><img src="' . $img . '" srcset="' . $img_2x . '" alt="loader"/>' ); // @codingStandardsIgnoreLine ?>
