...
function up_get_woo_version_number() {
	if ( WC()->version ) {
		return WC()->version;
	}
...
// function up_woocommerce_add_js_composer_shortcodes() {
...
// function up_woocommerce_output_related_products() {
// 	if ( porto_is_product() ) {
...
add_filter( 'wc_add_to_cart_message_html', 'uc_add_to_cart_message_html', 8, 3);
...
// rating
// function up_woocommerce_shop_loop_item_init_layout() {
...
// function up_woocommerce_shop_loop_item_reset_layout() {
...
// category
// function up_woocommerce_shop_loop_item_category() {
...
// function up_woocommerce_show_page_title( $args ) {
// 	return false;
// }

// function up_woocommerce_open_before_clearfix_div() {
...
function up_woocommerce_open_after_clearfix_div() {
...
function up_woocommerce_close_before_clearfix_div() {
	global $porto_shop_filter_layout;
	if ( ( ! isset( $porto_shop_filter_layout ) || 'horizontal2' != $porto_shop_filter_layout ) && ( ( function_exists( 'wc_get_loop_prop' ) && ! wc_get_loop_prop( 'is_paginated' ) ) || ! woocommerce_products_will_display() ) ) {
		return;
	}
...
function up_woocommerce_close_after_clearfix_div() {
	if ( ( function_exists( 'wc_get_loop_prop' ) && ! wc_get_loop_prop( 'is_paginated' ) ) || ! woocommerce_products_will_display() ) {
		return;
	}
	echo '</div>';
...
function up_grid_list_toggle( $el_class = false ) {
	global $woocommerce_loop, $porto_woocommerce_loop;
	if ( ! porto_is_elementor_preview() && ( ( function_exists( 'wc_get_loop_prop' ) && ! wc_get_loop_prop( 'is_paginated' ) ) || ! woocommerce_products_will_display() || isset( $porto_woocommerce_loop['view'] ) ) ) {
		return;
	}
...
function up_loop_shop_per_page() {
	global $porto_settings;
...
function up_woocommerce_available_variation( $variations, $product, $variation ) {
...
function up_layered_nav_link( $link, $term = false ) {
...
function up_loop_product_thumbnail() {
...
function up_widget_product_thumbnail() {
	global $porto_settings;
	$id               = get_the_ID();
...
function up_remove_related_products( $args ) {
	global $porto_settings;
...
function up_woocommerce_header_add_to_cart_fragment( $fragments ) {
...
function up_woocommerce_hide_update_notice( $flag, $notice ) {
	if ( 'update' === $notice ) {
		return false;
	}
	return $flag;
...
if ( wp_doing_ajax() ) {
	...
	function up_refresh_cart_fragment() {
		//check_ajax_referer( 'porto-nonce', 'nonce' );
		// phpcs:disable WordPress.Security.NonceVerification.NoNonceVerification
		$cart_ajax = new WC_AJAX();
		$cart_ajax->get_refreshed_fragments();
	...
	function up_refresh_wishlist_count() {
		//check_ajax_referer( 'porto-nonce', 'nonce' );
		// phpcs:disable WordPress.Security.NonceVerification.NoNonceVerification
...
function up_get_products_by_ids( $product_ids ) {
...
function up_get_rating_html( $product, $rating = null ) {
...
function up_woo_review_display_gravatar_wrap_start() {
	echo '<div class="img-thumbnail">';
}
function up_woo_review_display_gravatar_wrap_end() {
	echo '</div>';
}
add_filter( 'woocommerce_review_gravatar_size', 'porto_woo_review_gravatar_size' );
function up_woo_review_gravatar_size( $size ) {
	return '80';
}
// Quick View Html
...
function up_woocommerce_category_image() {
...
function up_woocommerce_shop_loop_item_title_open() {
	global $porto_settings;
...
function up_woocommerce_shop_loop_item_title_close() {
	?>
	</a>
	<?php
}
function up_woocommerce_shop_loop_item_title() {
	echo '<h3 class="woocommerce-loop-product__title">';
	the_title();
	echo '</h3>';
}
function up_woocommerce_single_excerpt() {
	global $post;
...
function up_woocommerce_next_product( $in_same_cat = false, $excluded_categories = '' ) {
	porto_adjacent_post_link_product( $in_same_cat, $excluded_categories, false );
}
function up_woocommerce_prev_product( $in_same_cat = false, $excluded_categories = '' ) {
	porto_adjacent_post_link_product( $in_same_cat, $excluded_categories, true );
}
function up_adjacent_post_link_product( $in_same_cat = false, $excluded_categories = '', $previous = true ) {
	...
	if ( isset( $porto_settings['catalog-enable'] ) && $porto_settings['catalog-enable'] ) {
		if ( $porto_settings['catalog-admin'] || ( ! $porto_settings['catalog-admin'] && ! ( current_user_can( 'administrator' ) && is_user_logged_in() ) ) ) {
			...
			if ( ! $porto_settings['catalog-review'] ) {
				...
				function up_woocommerce_remove_reviews_tab( $tabs ) {
					unset( $tabs['reviews'] );
					return $tabs;
				}
			}
...
function up_woocommerce_template_single_add_to_cart() {
...
function up_woocommerce_get_price_html_empty( $price, $product ) {
	return '';
}
function up_woocommerce_get_price_empty( $price, $param2, $param3 ) {
	return '';
}
function up_woocommerce_disable_rating( $false ) {
	return 'no';
}
function up_woocommerce_readmore_button() {
...
function up_shop_page_display_ajax( $value ) {
...
function up_woocommerce_term_metadata_ajax( $value, $object_id, $meta_key, $single ) {
...
function up_is_product_archive() {
	if ( is_archive() ) {
		$term = get_queried_object();
...
function up_woocommerce_stock_html( $availability_html, $product ) {
...
function up_woocommerce_add_stock_html() {
	global $product;
	if ( $product->is_type( 'simple' ) ) {
...
function up_woocommerce_init_layout() {
	if ( is_admin() ) {
		return;
	}
...
function up_woocommerce_template_single_custom_block() {
...
function up_woocommerce_custom_tabs( $tabs ) {
	global $porto_settings;
...
function up_woocommerce_global_tab( $tabs ) {
	global $porto_settings;
...
function up_woocommerce_custom_tab_content( $key, $tab ) {
...
function up_multi_currency_ajax( $actions ) {
...
*/
function up_wooc_extra_register_start_fields() {
	global $porto_settings;
...
* @return void
*/
function up_wooc_validate_extra_register_fields( $username, $email, $validation_errors ) {
	global $porto_settings;
	if ( isset( $porto_settings['reg-form-info'] ) && 'full' == $porto_settings['reg-form-info'] ) {
...
* @return void
*/
function up_wooc_save_extra_register_fields( $customer_id ) {
	global $porto_settings;
	if ( isset( $porto_settings['reg-form-info'] ) && 'full' == $porto_settings['reg-form-info'] ) {
...
function up_registration_errors_validation( $reg_errors, $sanitized_user_login, $user_email ) {
...
function up_shipping_calculator() {
...
function up_wc_body_class( $classes ) {
...
function up_woocommerce_review_order_before_payment() {
...
if ( class_exists( 'WC_Vendors' ) ) {
	...
	function up_add_to_author_profile( $contactmethods ) {
	...
	function up_wpvendors_product_seller_name() {
	...
	function up_wc_vendors_sold_by_meta() {
		global $porto_settings;
	...
	function up_user_profile_fields( $user ) {
	...
	function up_more_seller_product() {
		global $product;
...
function up_add_product_attribute_color( $attrs ) {
	return array_merge(
		$attrs,
		array(
...
function up_add_product_attribute_color_variation( $attribute_taxonomy, $i ) {
...
function up_woocommerce_dropdown_variation_attribute_options_html( $select_html, $args ) {
	global $porto_settings;
...
function up_woocommerce_layed_nav_color_html( $term_html, $term, $link, $count ) {
	$term_html   = '';
	$color_value = get_term_meta( $term->term_id, 'color_value', true );
	$label_value = get_term_meta( $term->term_id, 'label_value', true );
	$attrs       = '';
...
function up_yith_woocommerce_reset_filter_link( $link ) {
...
if ( defined( 'YITH_WCWL' ) ) {
	...
	function up_yith_wcwl_before_wishlist_view() {
		echo '<div class="align-left mt-3"><div class="box-content">';
	}

	function up_yith_wcwl_after_wishlist_view() {
		echo '</div></div>';
	}

	function up_yith_wcwl_edit_title_icon( $icon_html ) {
		return str_replace( 'fa fa-pencil"', 'fas fa-pencil-alt"', $icon_html );
	}

	function up_yith_wcwl_cancel_title_icon( $icon_html ) {
		return str_replace( 'fa fa-remove"', 'fas fa-times"', $icon_html );
	}

	function up_yith_wcwl_template_part_hierarchy( $arr, $template, $template_part, $template_layout, $var ) {
		return array(
	...
	function up_wcwl_remove_notice( $var ) {
...
// horizontal filter
function up_woocommerce_output_horizontal_filter() {
	global $porto_shop_filter_layout, $porto_settings;
	if ( porto_is_elementor_preview() && empty( $porto_shop_filter_layout ) ) {
		$porto_shop_filter_layout = $porto_settings['product-archive-filter-layout'];
	}
...
// sale product period
function up_woocommerce_sale_product_period() {
...
// get sale end date
function up_woocommerce_get_sale_end_date( $vars, $product, $variation ) {
...
function up_account_login_popup() {
	//check_ajax_referer( 'porto-nonce', 'nonce' );
...
function up_account_login_popup_login() {
...
function up_account_login_popup_register() {
...
function up_woocommerce_shortcodes_products() {
	//check_ajax_referer( 'porto-nonce', 'nonce' );
...
if ( ! function_exists( 'porto_woocommerce_subcategory_thumbnail' ) ) :
	function up_woocommerce_subcategory_thumbnail( $category ) {
...
function up_woocommerce_display_quantity_input_on_shop_page( $html, $product ) {
...
function up_cpo_add_to_cart_url( $add_to_cart_url, $product ) {
	if ( class_exists( 'Uni_Cpo' ) ) {
...
function up_cpo_add_to_cart_args( $args, $product ) {
...
function up_woocommerce_display_variation_on_shop_page( $el_class = '' ) {
...
if ( ! function_exists( 'porto_woocommerce_product_sticky_addcart' ) ) :
	 ...
	 *
	 */
	function up_woocommerce_product_sticky_addcart( $el_class = '' ) {
		global $porto_settings;
...
if ( ! function_exists( 'porto_woocommerce_add_to_cart_notification_html' ) ) :
	function up_woocommerce_add_to_cart_notification_html() {
		global $porto_settings;
		?>
...
if ( ! function_exists( 'porto_product_categories_add_featured' ) ) :
	function up_product_categories_add_featured( $category ) {
...
if ( ! function_exists( 'porto_woocommerce_get_catalog_ordering_args' ) ) :
	 ...
	 * Add sales order by
	 */
	function up_woocommerce_get_catalog_ordering_args( $args, $orderby, $order ) {
...
if ( ! function_exists( 'porto_woocommerce_widget_product_review_item_add_desc' ) ) :
	...
	function up_woocommerce_widget_product_review_item_add_desc( $args ) {
 ...
 */
function up_woocommerce_before_widget_product_review_list( $start_wrapper ) {
 ...
 * @return mixed
 **/
function uc_add_to_cart_message_html( $message, $products, $show_qty ) {
	$titles = array();
	$count  = 0;
	...
	/* translators: %s: product name */
	$added_text = sprintf( _n( '<strong class="single-cart-notice">%s</strong> <span class="font-weight-medium">ha sido añadido a tu carrito.</span>', '<strong class="single-cart-notice">%s</strong> <span class="font-weight-medium">ha sido añadido a tu carrito.</span>', $count, 'porto' ), wc_format_list_of_items( $titles ) );
...
if ( ! function_exists( 'porto_template_loop_compare' ) ) {
	function up_template_loop_compare( $extra_class = '', $hide_title = false, $icon_cls = '', $icon_pos = 'left', $icon_cls_added = '' ) {
		global $product;
...
if ( class_exists( 'WCFMmp' ) ) {
	...
	function up_store_sidebar_args( $args ) {
 ...
 * Track product views.
 */
function up_wc_track_product_view() {
